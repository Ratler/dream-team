...
- **`agents/*.md`** — Agent definitions with model, color, and behavioral instructions. Each agent's `model:` frontmatter is the single source of truth for model selection (defaults: builder=opus, researcher=sonnet, architect=opus, reviewer=sonnet, tester=sonnet, validator=haiku, debugger=opus, security-reviewer=opus, docs=sonnet, scout=haiku, merger=sonnet) — dispatches omit `model` so the definition applies and stays customizable in one place. Builder and debugger agents use `isolation: "worktree"` for git worktree isolation in delegated/team modes. Builder, reviewer, architect, and docs agents use `memory: project` for persistent cross-session knowledge. Read-only agents (researcher, architect, reviewer, security-reviewer, validator, scout) use `disallowedTools: Write, Edit, NotebookEdit` to enforce read-only access. All agents include `[agent-type: X]` in their report format and write reports to the task description via `TaskUpdate(description: ...)`. The reviewer agent runs a structural-quality pass (simplification, ~1k-line file-size threshold, spaghetti prevention, boundary leaks, and architect-design compliance) alongside its correctness, spec-compliance, and test review — acting as the post-build enforcement backstop for the architect's design. Structural regressions and unjustified design drift are Important (blocking); pure missed simplifications are Minor (mentioned, non-blocking).
